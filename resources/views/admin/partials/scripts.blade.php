...
{{-- <script>
  $(document).ready(function(){
    // make notification to read
    $(document).on("click", ".notification-item", function(){
      let notificationId = $(this).data('id');
      let notificationElement = $(this);

      $.ajax({
        url: '/admin/notification/' + notificationId + '/markasread',
        method: 'get',
        success: function(data) {
          notificationElement.removeClass('bg-light').addClass('bg-transparent');
          //$("#notificationsIconCounter").load(" #notificationIconCounter > *");

          let dot = $('#notificationsIconCounter');
          let count = parseInt(dot.text());
          if (!isNaN(count)) {
            if (count > 1) {
              dot.text(count - 1);
            } else {
              dot.remove(); // نخفيه إذا وصل 0
            }
          }
        },
        error: function() {
